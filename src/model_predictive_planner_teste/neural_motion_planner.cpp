...
void
compute_path_phis(vector<carmen_robot_and_trailer_path_point_t> *lane_in_local_pose)
{
...
get_trajectory_dimensions_from_robot_state(carmen_robot_and_trailer_pose_t *localizer_pose, Command last_odometry,	Pose *goal_pose)
{
	...
	printf("td.control_parameters.valid: %d\n", td.control_parameters.valid);//@CAIO: td.control_parameters.valid: 0
	return (td);
}
		...
		carmen_behavior_selector_path_goals_and_annotations_message *path_goals_and_annotations_message)
{
	...
	{
		printf("Entrou aqui no !set_reverse_planning_global_state\n");//@CAIO: não entrou aqui
		paths.clear();
		return (paths);
	}
	...
	if (GlobalState::use_path_planner || GlobalState::use_tracker_goal_and_lane)
	{
		...
		printf("entrou aqui no build_detailed_path_lane\n");//@CAIO: não entrou aqui
	}
	else
	{
		printf("entrou aqui no build_detailed_rddf_lane\n");//@CAIO: entrou aqui
		goal_in_lane = build_detailed_rddf_lane(goal_pose, &lane_in_local_pose, detailed_lane);
		printf("goal_in_lane: %d\n", goal_in_lane);//@CAIO: goal_in_lane: 1
		if (!goal_in_lane)
			detailed_lane.clear();
	}
	...
	printf("td.dist: %lf\n", td.dist);//@CAIO: td.dist: 0.000000
	printf("td.theta: %lf\n", td.theta);//@CAIO: td.theta: 0.000000
	printf("td.d_yaw: %lf\n", td.d_yaw);//@CAIO: td.d_yaw: 0.000000
	printf("td.phi_i: %lf\n", td.phi_i);//@CAIO: td.phi_i: 0.000000
	printf("td.v_i: %lf\n", td.v_i);//@CAIO: td.v_i: 0.000000
	printf("td.beta_i: %lf\n", td.beta_i);//@CAIO: td.beta_i: 0.000000
	printf("td.goal_pose.x: %lf\n", td.goal_pose.x);//@CAIO: td.goal_pose.x: 0.000000
	printf("td.goal_pose.y: %lf\n", td.goal_pose.y);//@CAIO: td.goal_pose.y: 0.000000
	printf("td.goal_pose.theta: %lf\n", td.goal_pose.theta);//@CAIO: td.goal_pose.theta: 0.000000
	printf("td.goal_pose.beta: %lf\n", td.goal_pose.beta);//@CAIO: td.goal_pose.beta: 0.000000
	printf("td.control_parameters.valid: %d\n", td.control_parameters.valid);//@CAIO: td.control_parameters.valid: 0
	printf("td.control_parameters.valid_quest %s\n", td.control_parameters.valid ? "true" : "false"); //@CAIO: td.control_parameters.valid: 0
	printf("td.control_parameters.sf: %lf\n", td.control_parameters.sf);//@CAIO: td.control_parameters.sf: 0.000000
	printf("td.control_parameters.vf: %lf\n", td.control_parameters.vf);//@CAIO: td.control_parameters.vf: 0.000000
	printf("td.control_parameters.a: %lf\n", td.control_parameters.a);//@CAIO: td.control_parameters.a: 0.000000
	printf("td.control_parameters.s: %lf\n", td.control_parameters.s);//@CAIO: td.control_parameters.t: 0.000000
	printf("td.control_parameters.tt: %lf\n", td.control_parameters.tt);//@CAIO: td.control_parameters.t: 0.000000
	printf("td.control_parameters.k: %lf\n", td.control_parameters.k);//@CAIO: td.control_parameters.sf: 0.000000
	printf("use_lane: %d\n", use_lane);//@CAIO: use_lane: 1
	printf("target_v: %lf\n", target_v);//@CAIO: target_v: 0.000000
	printf("detailed_lane.size(): %lu\n", detailed_lane.size());//@CAIO: detailed_lane.size(): 0
	printf("lane_in_local_pose.size(): %lu\n", lane_in_local_pose.size());//@CAIO: lane_in_local_pose.size(): 0
	...
	if (otcp.valid) //@CAIO: comentei aqui
	{ //@CAIO: comentei aqui
		vector<carmen_robot_and_trailer_path_point_t> path;
		vector<carmen_robot_and_trailer_path_point_t> path_local;

		if (!get_path_from_optimized_tcp(path, path_local, otcp, td, localizer_pose))//@CAIO: comentei aqui
		{//@CAIO: comentei aqui
			paths.clear();//@CAIO: comentei aqui
			return (paths);//@CAIO: comentei aqui
		}//@CAIO: comentei aqui
		paths[0] = path;
		fflush(stdout);

		previous_good_tcp = otcp;//@CAIO: comentei aqui
		last_timestamp = path_goals_and_annotations_message->timestamp;
	}//@CAIO: comentei aqui
	else// @CAIO: comentei aqui
	{ //@CAIO: comentei aqui
		if ((path_goals_and_annotations_message->timestamp - last_timestamp) > 0.5) //@CAIO: comentei aqui
			previous_good_tcp.valid = false; //@CAIO: comentei aqui
