		...
		TrajectoryControlParameters &tcp, double v0, double i_beta, double delta_t)
{
	...
	//td.phi_i = tcp.k[0]; @CAIO: comentei aqui
	td.v_i = v0;
	tcp.vf = command.v;
...
//		(GlobalState::behavior_selector_task == BEHAVIOR_SELECTOR_PARK))
	...
	TrajectoryControlParameters tcp_complete = get_optimized_trajectory_control_parameters(tcp_seed, params);//@CAIO: comentei aqui(alterei para tcp_complete = tcp_seed)
#ifdef PUBLISH_PLAN_TREE
